...
def create_mcp_router() -> APIRouter:
    ...
    @router.get("", summary="Public MCP Central discovery")
    ...
    async def _handle_request(
        ...
        try:
            response = await _dispatch(body, server_names, hidden_tools, db)
...
def _extract_api_key(request: Request) -> str | None:
    ...
    query_val = request.query_params.get("api_key")
    if header_val:
        ...
        server_entries.append(
            {
                ...
                "tools": _namespaced_tools_for_discovery(mcp_router, [server.name]),
            }
        )
    ...
    group_entries = [
        {
            ...
            "tools": _namespaced_tools_for_discovery(
                mcp_router,
                [server.name for server in servers if server.group_id == group.id],
...
def _namespaced_tools_for_discovery(
    ...
    if mcp_router is None:
        return []
    return mcp_router.get_all_namespaced_tools(
        group_server_names=server_names,
